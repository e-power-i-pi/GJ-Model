...
for i in alpha:
    for j in theta:
        f = pd.read_csv(path+"Results_"+str(i)+"/B=14_theta="+str(j)+"_deg_plas_alpha_"+str(i)+"_deg_profile.dat",
            header = None)
        # print(np.where(alpha == i)[0], np.where(theta == j)[0])
        #avgP[np.where(alpha == i)[0]][np.where(theta == j)[0]] = np.mean(np.array(f))
...
ani.save("|nGJ| vs alpha.gif", writer='imagemagick', fps = 2)
...
#             period = 1
#             omega = 2*np.pi / period
#             x1, x2, rf = MagF(0, [choice, theta[i], phi[j]], period, 10, 1e10, 3 * clock * np.pi/180)
#             nGJ[i][j] = (2 * np.dot(x1,x2) / e) * rf   # (2*np.pi*c))
